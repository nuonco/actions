...
helm and aws-iam-authenticator for EKS, using the runner-injected kubeconfig.

| | |
| --- | --- |
| image | `nuonco/helm-aws` |
...
| dockerfile | [`images/helm/aws/Dockerfile`](./Dockerfile) |
| status | experimental |
...
name    = "helm-releases"
timeout = "5m"
image   = "{{.nuon.components.helm-aws.outputs.image.ref}}"
...
- No `kubectl` here. Use `kubectl-aws` or `terraform-aws` when a step needs both.
