...
helm and gke-gcloud-auth-plugin for GKE, using the runner-injected kubeconfig.

| | |
| --- | --- |
| image | `nuonco/helm-gcp` |
...
| dockerfile | [`images/helm/gcp/Dockerfile`](./Dockerfile) |
| status | experimental |
...
name    = "helm-releases"
timeout = "5m"
image   = "{{.nuon.components.helm-gcp.outputs.image.ref}}"
...
- No `kubectl` here. Use `kubectl-gcp` or `terraform-gcp` when a step needs both.
