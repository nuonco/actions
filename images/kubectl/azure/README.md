...
kubectl and kubelogin for AKS, using the runner-injected kubeconfig.

| | |
| --- | --- |
| image | `nuonco/kubectl-azure` |
...
| dockerfile | [`images/kubectl/azure/Dockerfile`](./Dockerfile) |
| status | experimental |
...
[public]
image_url = "nuonco/kubectl-azure"
tag       = "latest"
...
name    = "list-nodes"
timeout = "5m"
image   = "{{.nuon.components.kubectl-azure.outputs.image.ref}}"
...
- Scoped to AKS work from an action: `kubectl` and the auth plugin on Alpine, nothing else.
